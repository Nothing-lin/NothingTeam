...
public class DiffUtilCallback extends DiffUtil.Callback {
    ...
    private List<ToolTabCardInfo> mOldData;
    private List<ToolTabCardInfo> mNewData;

    public DiffUtilCallback(List<ToolTabCardInfo> oldData, List<ToolTabCardInfo> newData) {
        mOldData = oldData;
        mNewData = newData;
    }
    ...
    public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {
        ToolTabCardInfo oldInfo = mOldData.get(oldItemPosition);
        ToolTabCardInfo toolTabCardInfo = mNewData.get(newItemPosition);
        //如果有内容不同，就返回false
        if (!StringUtils.equals(oldInfo.getUserName(), toolTabCardInfo.getUserName())) {
            return false;
        }
        if (oldInfo.getPraise() != toolTabCardInfo.getPraise()) {
            return false;
        }
        if (oldInfo.getComment() != toolTabCardInfo.getComment()) {
            return false;
        }
        if (oldInfo.getRead() != toolTabCardInfo.getRead()) {
            return false;
        }
        return true;
    }
    ...
    public Object getChangePayload(int oldItemPosition, int newItemPosition) {
        ...
        if (!StringUtils.equals(oldInfo.getUserName(), toolTabCardInfo.getUserName())) {
            payload.putString(PAYLOAD_USER_NAME, toolTabCardInfo.getUserName());
        }
        if (oldInfo.getPraise() != toolTabCardInfo.getPraise()) {
            payload.putInt(PAYLOAD_PRAISE, toolTabCardInfo.getPraise());
        }
        if (oldInfo.getComment() != toolTabCardInfo.getComment()) {
            payload.putInt(PAYLOAD_COMMENT, toolTabCardInfo.getComment());
        }
        if (oldInfo.getRead() != toolTabCardInfo.getRead()) {
            payload.putInt(PAYLOAD_READ_NUMBER, toolTabCardInfo.getRead());
        }
        if (payload.size() == 0) {
            return null;
        }
